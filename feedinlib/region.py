import xarray as xr # todo: in setup.py?
import numpy as np
import pandas as pd
...
class Region:
    ...
    def wind_feedin(self, register, assignment_func=None, snapshots=None,
        ...
        tools.add_weather_locations_to_register(
            register=register, weather_coordinates=self.weather) # todo line below! - now just dummy weather
        ...
        weather_locations = register[['weather_lat', 'weather_lon']].groupby(['weather_lat', 'weather_lon']).size().reset_index().drop([0], axis=1)
